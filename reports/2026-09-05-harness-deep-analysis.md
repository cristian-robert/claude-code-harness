...
**1.3 Partial-output marking needs Claude Code ≥ 2.1.246.** `scout` (25), `qa-evaluator` (50) and `research-gatherer` (20) set `maxTurns`; the cap is enforced on every version, but below 2.1.246 a capped subagent's output comes back looking finished — no partial marker, no resume hint — so a truncated scout return reads as a complete answer. State a minimum Claude Code version in README and have `/harness-init` step 4 record `claude --version`. *Corrected 2026-09-05 against the raw `sub-agents.md`: the first draft, taken from a summarized fetch, said the cap itself was ignored — section 5.1's own failure mode.*
