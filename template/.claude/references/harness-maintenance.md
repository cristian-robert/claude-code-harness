...
| Agents | `.claude/agents/*.md` | Subagent definitions; dispatches pin a ROLE — `tier:` (`scout`/`routine`/`build`/`deep`) + `effort:`, never a model name. `.claude/harness.json` → `models` is the one place a model is named; `/models` re-verifies it |
