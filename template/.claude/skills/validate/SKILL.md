...
Runs the FULL gate: every check in the "Commands" table of CLAUDE.md (lint, typecheck, unit tests, build when the diff touches app code). Superset of the cheap Stop-hook gate in `.claude/harness.json` — passing that gate is not passing this one.
...
`$ARGUMENTS` if given, else the plan referenced by the newest `reports/*-implementation-report.md`. Plan has `item:` → hold that item path for the Log append in the Output contract (independent of whether the optional runtime lens in 3b fires) so the QA event always lands in the audit trail.

## 1 · Determine the gate

| Source | Checks |
|---|---|
| CLAUDE.md "Commands" table | lint, typecheck, unit tests — always |
