...
- `examples/`: copy `frontend.AGENTS.md` / `backend.AGENTS.md` into real subdirs only where a dir has local traps; delete `examples/` if unused. `guard.mjs` denies `rm -rf`/`find -delete`/`git clean -d` — remove each file explicitly, then `rmdir` the empty dir. Same for every prune below.
