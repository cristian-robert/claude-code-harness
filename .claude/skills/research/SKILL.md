...
## 3 · Gather (orchestrator directs, `routine` tier gathers)
...
and dispatch `research-gatherer` (`routine` tier) to read official docs + web. It returns structured, sourced
...
Autonomous mode (declared per `.claude/references/autonomous-mode.md` — being invoked from `/plan-work` is not a declaration): no interactive
