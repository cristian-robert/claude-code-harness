...
- `.claude/harness.json` still has an `autonomous` key (pre-3.3.0 install, or hand-copied)? DELETE it and print one line — `harness.json: removed inert "autonomous" key — autonomy is declared per session, in your own words (.claude/references/autonomous-mode.md).` `init`/`update` already strip it, so a survivor means the file was edited around them; never re-add it.
...
- `.gitignore`: add `.claude/state/` + `.worktrees/` + `.superpowers/` + `knowledge-base/.obsidian/` — runtime state, /implement's in-repo worktrees, the superpowers SDD workspace, and the operator's Obsidian workspace never commit (spec decision 24: no `.obsidian/` ships; the operator opens the folder).
...
| `claude --version` | ≥ 2.1.246 — below it a subagent that hits `maxTurns` returns output that looks finished (no partial marker, no resume hint); record the version |
